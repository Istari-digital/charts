...
This is the `-sec` fork, so the chart ships hardened by default. You do not have to
turn anything on to get the baseline below; it applies to the Alpha, Zero, and Ratel
...
Four things stay **off** by default, because each one needs a deliberate decision or
some external material (a key, a certificate, a Secret). Turn them on for a
production posture:

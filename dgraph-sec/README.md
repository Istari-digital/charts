...
![Version: 0.8.0](https://img.shields.io/badge/Version-0.8.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: v25.3.8-sec.0.2.3](https://img.shields.io/badge/AppVersion-v25.3.8--sec.0.2.3-informational?style=flat-square)
...
but are not evaluated** — any permission check that traverses a caveated relationship
returns `NO_PERMISSION`, so a caveated relationship never grants access (do not model
a permission that depends on one); the `WatchService` returns `Unimplemented`, and
`WriteSchema` is additive-only (definitions are never removed).

ZedTokens are likewise dgraph-sec's own format: SpiceDB-issued tokens are rejected
with `INVALID_ARGUMENT`, so a deployment migrating from SpiceDB must purge any
persisted tokens (client caches, stored watermarks) at cutover. An upcoming release
softens this — an undecodable `at_least_as_fresh` token will degrade to a fresh-local
read, since the freshness floor is a best-effort hint — but `at_exact_snapshot` will
keep rejecting foreign tokens, as repeatable-read semantics cannot be faked against a
token dgraph-sec cannot decode.

Turn it on with `alpha.zanzibar.enabled: true` and `mode: preshared`. In production,
point `existingSecret` at a Terraform-managed Secret that holds the preshared key
under the `zanzibar` data key. For a local run, supply the key inline under `file`
instead; the chart then creates the Secret for you, named
`dgraph-sec-alpha-zanzibar-secret`:

```yaml
# values.yaml — enable the Zanzibar/SpiceDB API (dev: chart creates the key Secret)
alpha:
  zanzibar:
    enabled: true
    mode: preshared               # "open" disables auth entirely; dev only, guarded by allowInsecureOpen
    file:
      zanzibar: <base64 preshared key>   # chart creates Secret dgraph-sec-alpha-zanzibar-secret
    # production: omit `file` and set existingSecret to your Terraform-managed Secret
    # existingSecret: <your-zanzibar-secret>
```
...
With the API enabled, manage the schema, relationships, and permission checks with
[`zed`](https://github.com/authzed/zed) over a port-forward. Read the preshared key
from its Secret, forward Alpha's gRPC port, then point a `zed` context at it:

```sh
NS=<namespace>                            # e.g. istari
SECRET=dgraph-sec-alpha-zanzibar-secret   # chart-created dev Secret; with existingSecret, use that name

# Read the zanzibar preshared key from the cluster Secret.
# The data key is alpha.zanzibar.keyName (default: zanzibar).
KEY="$(kubectl -n "$NS" get secret "$SECRET" -o jsonpath='{.data.zanzibar}' | base64 -d)"

# Forward Alpha's client gRPC port.
kubectl -n "$NS" port-forward svc/dgraph-sec-alpha 9080:9080 &

# Point zed at it. Drop --insecure once you connect over native TLS or through the mesh.
zed context set dgraph localhost:9080 "$KEY" --insecure
```

With the context set, dump the schema, restore a schema, or run a permission check:

```sh
# Dump the current schema to a file.
zed schema read > schema.zed

# Restore (write) a schema from a file. WriteSchema is additive: it creates or updates
# the definitions in the file, but never deletes definitions already present.
zed schema write schema.zed

# Check a permission: may user:alice view document:readme?
...
> This chart adds no dedicated Zanzibar ingress or Service, but Zanzibar shares Alpha's
> 9080: exposing that port through `alpha.service.type` (`LoadBalancer`/`NodePort`) or
> `alpha.ingress_grpc.enabled` exposes the Zanzibar API **and** the native Dgraph gRPC
> API together. The defaults keep both in-cluster — see
> [Zanzibar authorization API](./docs/topology.md#zanzibar-authorization-api).
...
| `image.tag` | `v25.3.8-sec.0.2.3` | Image tag (tracks `Chart.appVersion`). |
...
4. Install. `--wait` brings up Alpha and Zero, then the ACL bootstrap rotates groot
   ...
   > `helm test` in the next step hang. Upgrade Helm to resolve it:
   > `brew upgrade helm` (macOS), or
   > `sudo apt-get update && sudo apt-get install --only-upgrade helm` (Debian/Ubuntu,
   > via the [Helm apt repo](https://helm.sh/docs/intro/install/#from-apt-debianubuntu)).
...
| alpha.initContainers | object | `{"init":{"command":["bash","-c","trap \"exit\" SIGINT SIGTERM\necho \"Write to /dgraph/doneinit when ready.\"\nuntil [ -f /dgraph/doneinit ]; do sleep 2; done\n"],"enabled":false,"env":[],"envFrom":[],"image":{"<<":{"debug":false,"pullPolicy":"IfNotPresent","registry":"istaridigital.jfrog.io","repository":"main-docker-local/dgraph-sec","tag":"v25.3.8-sec.0.2.3"}}}}` | Optional alpha pre-start init container (e.g. restore or bulk load before Alpha starts). |
...
| alpha.initContainers.init.image.<<.tag | string | `"v25.3.8-sec.0.2.3"` | dgraph-sec image tag (matches the chart appVersion). |
...
| backups.image | object | `{"<<":{"debug":false,"pullPolicy":"IfNotPresent","registry":"istaridigital.jfrog.io","repository":"main-docker-local/dgraph-sec","tag":"v25.3.8-sec.0.2.3"}}` | Image for backup CronJobs (defaults to the shared dgraph-sec image; needs curl). |
...
| backups.image.<<.tag | string | `"v25.3.8-sec.0.2.3"` | dgraph-sec image tag (matches the chart appVersion). |
...
| image.tag | string | `"v25.3.8-sec.0.2.3"` | dgraph-sec image tag (matches the chart appVersion). |

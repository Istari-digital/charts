...
Flags:
 ...
 -r, --release             Helm Chart Release Name (required)
  ...
  -z, --zero                Create certificates for zero as well
     --fullname           Helm fullnameOverride value (default mirrors dgraph.fullname: \$RELEASE-dgraph-sec truncated to 24 chars)
USAGE
}

######
# check_environment - verify dgraph-sec binary exists
##########################
check_environment() {
  ## Check for dgraph-sec command
  command -v dgraph-sec > /dev/null || \
    { echo "[ERROR]: 'dgraph-sec' command not found" 1>&2; exit 1; }
}
...
parse_command() {
  ...
  if ! PARSED_ARGUMENTS=$(
    ...
    --long client:,domain:,debug,extra:,fullname:,help,namespace:,release:,replicas:,tls_dir:,zero \
...
get_node_list() {
  ...
  local FULLNAME="${FULLNAME:-$(printf '%s-%s' "$RELEASE" "dgraph-sec" | cut -c1-24)}"

  ## Build List
  for (( IDX=0; IDX<REPLICAS; IDX++ )); do
    ## Add service FQDN for each dgraph pod
    LIST+=("$FULLNAME-$TYPE-$IDX.$FULLNAME-$TYPE-headless.$NAMESPACE.svc.$LOCAL_DOMAIN")
  done

  ## Add service FQDN for service
  LIST+=("$FULLNAME-$TYPE-headless.$NAMESPACE.svc.$LOCAL_DOMAIN")
...
create_certificates() {
  ...
  # Make Alpha Keys/Certs
  mkdir -p "$TLS_DIR"/alpha
  # shellcheck disable=SC2086
  dgraph-sec cert --nodes "$ALPHA_LIST" $CLIENT_OPT --dir "$TLS_DIR"/alpha
  ...
  if [[ $ZERO_ENABLED == "true" ]]; then
    ...
    ## Make Zero Keys/Cert
    dgraph-sec cert --nodes "$ZERO_LIST" --dir "$TLS_DIR"/zero
  fi
}

...
{{/*
Per-workload identity env, rendered (via "istari-platform.containerEnv") for the containers that
call it — the OTEL-emitting services fileservice, identity, mcp, and secure-connection (web, init,
and migration Job), the identity registration Jobs, and the API Gateway proxy. frontend and docs
render their env independently and receive no identity block. Always emits four downward-API
building blocks — POD_NAME,
...
contract lives, shared by the containers that call it — the OTEL-emitting services (web, init,
and migration Job), the identity registration Jobs, and the API Gateway proxy (frontend and docs
render their env independently and receive no identity block):
...
identity service values. Emitted as explicit `env` (not the ConfigMap) so each
value wins over any key of the same name in `secretName`: envFrom applies the
Secret after the ConfigMap, but an explicit `env` entry beats both. That removes
the "a stale OIDC key in the Secret silently overrides the values" foot-gun — the
values are authoritative for the non-secret OIDC config. The client secret /
private key stay Secret-only (OIDC_CLIENT_SECRET / OIDC_PRIVATE_KEY).

Each field emits only when non-empty, so an empty block adds nothing. provider and
clientAuthMethod are validated against the supported enums, so a typo fails
`helm template` rather than the running pod.
*/}}
{{- define "istari-platform.identityOidcEnv" -}}
{{- $oidc := dig "oidc" dict . -}}
{{- $provider := dig "provider" "" $oidc -}}
...
{{- $authMethod := dig "clientAuthMethod" "" $oidc -}}

{{/*
Route table for the API Gateway, shared by the proxy ConfigMap and NOTES.txt.
Emits a JSON array of {prefix, service, port} objects.

Routes for the platform's own services are managed by the chart: per the
service-router engspec, adding a platform service behind the API Gateway is a chart
...
backend is not deployed answers 502 at the API Gateway. `apiGateway.extraRoutes` entries are
...
{{- range $entry := .Values.apiGateway.extraRoutes -}}
{{- range $key, $unused := $entry -}}
{{- if not (has $key (list "prefix" "service" "port")) -}}
{{- fail (printf "apiGateway.extraRoutes: unsupported key %q (entry with prefix %q) — entries accept only prefix, service, and port, and the target Service must be in this release's namespace" $key (default "<unset>" $entry.prefix)) -}}
{{- end -}}
{{- end -}}
{{- $prefix := required "apiGateway.extraRoutes: every entry needs a prefix" .prefix -}}
{{- if not (regexMatch "^(/[a-zA-Z0-9_-]+)+$" $prefix) -}}
{{- fail (printf "apiGateway.extraRoutes: prefix %q is invalid — it must start with \"/\", may not end with \"/\", and its segments may contain only letters, digits, \"-\", and \"_\"" $prefix) -}}
{{- end -}}
{{- if eq $prefix "/healthz" -}}
{{- fail "apiGateway.extraRoutes: /healthz is reserved for the API Gateway's own health endpoint" -}}
{{- end -}}
{{- $service := required (printf "apiGateway.extraRoutes: service is required for prefix %s" $prefix) .service | toString -}}
{{- if or (gt (len $service) 63) (not (regexMatch "^[a-z]([-a-z0-9]*[a-z0-9])?$" $service)) -}}
{{- fail (printf "apiGateway.extraRoutes: service %q (prefix %s) is not a valid Kubernetes Service name (a DNS-1035 label: max 63 chars, lowercase letters, digits, and hyphens, starting with a letter). Use the Service's short name only — dotted/FQDN and cross-namespace targets are not supported, and the Service must live in this release's namespace" $service $prefix) -}}
{{- end -}}
{{- $rawPort := required (printf "apiGateway.extraRoutes: port is required for prefix %s" $prefix) .port -}}
{{- $port := int $rawPort -}}
{{- if or (lt $port 1) (gt $port 65535) (ne ($rawPort | toString) ($port | toString)) -}}
{{- fail (printf "apiGateway.extraRoutes: port %v (prefix %s) must be a whole number between 1 and 65535" $rawPort $prefix) -}}
{{- end -}}
...
{{- fail (printf "api-gateway routes: prefix %s is declared more than once (check apiGateway.extraRoutes against the chart-managed routes)" .prefix) -}}
{{- end -}}
...
{{/*
Effective tracing setting for the API Gateway, as the string "true" or "false".
`apiGateway.tracing.enabled` is tri-state: an explicit true/false wins; unset/null means
...
*/}}
{{- define "api-gateway.tracing.enabled" -}}
{{- $jaegerEnabled := dig "enabled" false (default dict .Values.jaeger) -}}
{{- $setting := dig "tracing" "enabled" "" (default dict .Values.apiGateway) -}}
...
{{- fail (printf "apiGateway.tracing.enabled must be true, false, or left unset (null); got %q — quoted strings are not booleans" ($setting | toString)) -}}
{{- end -}}
{{- end }}

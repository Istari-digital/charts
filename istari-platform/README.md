...
| identityService.frontendClientRegistration.enabled | bool | `false` | Whether to render the registration Job. Off by default: only enable in environments where `identityService.secretName` carries `ISTARI_DIGITAL_IDENTITY_SERVICE_FRONTEND_CLIENT_ID` / `_FRONTEND_REDIRECT_URIS`; otherwise the hook Job fails the release with a missing-secret-key error. Also requires an identity-service image whose `register-client` supports `-public-client-id` (identity-service PR #88). |

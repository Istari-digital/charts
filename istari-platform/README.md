...
| identity.oidc | object | (see fields below) | Upstream OIDC / IdP configuration — the NON-secret half of pointing identity at an IdP. Each field set here renders into the env ConfigMap as the matching `ISTARI_DIGITAL_IDENTITY_SERVICE_*` var, so switching IdP (e.g. Zitadel → Keycloak) is a values change applied in the same `helm upgrade` that enables `idpMigration`. The client SECRET is NOT here — it stays in `secretName` (`ISTARI_DIGITAL_IDENTITY_SERVICE_OIDC_CLIENT_SECRET`, or `..._OIDC_PRIVATE_KEY` for private_key_jwt). IMPORTANT: `envFrom` applies `secretName` AFTER this ConfigMap, so keep the non-secret OIDC keys below OUT of that Secret — a stale key there (e.g. the old issuer) would override the value set here. Each field renders only when non-empty; an empty block preserves today's fully Secret-driven configuration. |
| identity.oidc.clientAuthMethod | string | `""` | Client auth method at the IdP token endpoint: `private_key_jwt` (default, needs `..._OIDC_PRIVATE_KEY` in the Secret), `client_secret_basic`, or `client_secret_post` (both need `..._OIDC_CLIENT_SECRET` in the Secret). |
...
| identity.oidc.provider | string | `""` | Upstream IdP provider implementation: `zitadel` (default in-app) or `keycloak`. Selects the `internal/idp` wiring (role lookups, user lookup, token refresh). One IdP per deployment. |

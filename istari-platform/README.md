...
MCP consumes the gateway as a client; the gateway does not serve MCP requests. No `/mcp` route exists, so `https://<your-api-host>/mcp` returning 404 is expected, not a misconfiguration.
...
Routes for the platform's services are managed by the chart and always present, with no configuration needed. Additional prefixes for in-cluster services the chart does not deploy can be attached via `apiGateway.extraRoutes`. Enabling the API Gateway is one line in your values file:
...
Expose it the same way as the registry service: point your external load balancer or existing ingress controller at the API Gateway's `kind: Service` on port 80, terminating TLS there (the API Gateway itself serves plain HTTP and requires no Istio). The chart can also create a Kubernetes Ingress (`apiGateway.ingress`) or Istio VirtualService (`apiGateway.virtualService`) for you. Optional per-request tracing lights up automatically when the bundled Jaeger is enabled (`jaeger.enabled`); see `apiGateway.tracing.enabled`.
...
|-----|------|---------|-------------|
| apiGateway.affinity | object | `{}` | Affinity |
| apiGateway.apiUrl | string | `""` | Public base URL of this environment's API Gateway, with no path (e.g. `https://api.your-domain.com`). Setting it turns on the gateway environment-variable contract: the registry and frontend address the registry through `<base>/registry`, and the identity-service publishes and accepts the gateway-prefixed URL. Leave empty to keep every client on the per-service hostnames. This chart never derives a default for this value from another (including `apiGateway.ingress`), so a release that deploys the API Gateway still needs `apiUrl` set explicitly to turn the contract on. Trailing slashes and surrounding whitespace are stripped. |
| apiGateway.autoscaling.cpuUtilization | int | `80` | Average CPU utilization percentage. Set to `null` to disable. |
| apiGateway.autoscaling.enabled | bool | `false` | Enable/Disable autoscaling |
| apiGateway.autoscaling.maxReplicas | int | `3` | Maximum number of replicas |
| apiGateway.autoscaling.memoryUtilization | int | `80` | Average Memory utilization percentage. Set to `null` to disable. |
| apiGateway.autoscaling.minReplicas | int | `2` | Minimum number of replicas. Defaults to 2 for the same reason as `apiGateway.replicaCount`: the API Gateway is the single entry point for all routed traffic. |
...
| apiGateway.extraRoutes | list | `[]` | Additional path-prefix routes served alongside the chart-managed ones, for reaching in-cluster services this chart does not deploy. Each entry maps a URL path prefix to a Kubernetes Service in the same namespace as this release (cross-namespace and FQDN targets are not supported): the API Gateway matches the prefix and everything under it (never other prefixes that merely share its leading characters), strips it, and forwards the request to `service:port` over plain HTTP. A prefix must start with `/`, must not end with `/`, may use letters, digits, `-`, `_`, and `/` segment separators, and may not duplicate a chart-managed route or the reserved `/healthz` — invalid entries fail the render. When prefixes overlap, the longest match wins. Most deployments should leave this empty: routes for platform services are built into the chart and appear automatically. |
| apiGateway.image | string | `"istaridigital.com/caddy-fips"` | Image name. The combination of registry, image, and tag will be used to pull the image. Defaults to the Chainguard FIPS Caddy variant in the Istari customer-docker JFrog repo, pulled with the same credentials as the rest of the chart. The `istaridigital.com/` path segment is part of the repository path — not a typo. |
| apiGateway.imagePullPolicy | string | `"IfNotPresent"` | Image pull policy |
| apiGateway.ingress.annotations | object | `{}` | Annotations on the Ingress. Use this for controller-specific behavior (cert-manager, nginx, ALB, etc.). |
| apiGateway.ingress.className | string | `""` | `ingressClassName` on the Ingress. Leave empty to use the cluster's default IngressClass. |
| apiGateway.ingress.enabled | bool | `false` | Create a Kubernetes Ingress for this service. The cluster must have an Ingress controller (nginx, ALB / EKS Auto Mode, GCE, Traefik, etc.) that watches the chosen IngressClass. |
| apiGateway.ingress.hosts | list | `[]` | One entry per `spec.rules[]`. Empty by default so that enabling the Ingress without supplying your real API hostname fails the render instead of shipping a rule for a hostname that isn't yours. `host` is optional within an entry — omit it to match any hostname. |
| apiGateway.ingress.labels | object | `{}` | Additional labels on the Ingress (in addition to the standard api-gateway labels). |
| apiGateway.ingress.servicePort | int | `80` | Service port the Ingress targets. Defaults to 80. |
| apiGateway.ingress.tls | list | `[]` | TLS configuration; passed through to `spec.tls[]` verbatim. TLS terminates at the Ingress controller — the API Gateway behind it serves plain HTTP. Example: `tls: [{hosts: [api.customer-domain.com], secretName: api-tls-cert}]`, where the Secret is a standard TLS Secret (`kubectl create secret tls ...`) or is issued by cert-manager via annotations above. |
| apiGateway.listen | int | `8080` | Port the proxy container listens on. The API Gateway's `kind: Service` receives traffic on port 80 and forwards it here; you should never need to change this. |
| apiGateway.nodeSelector | object | `{}` | Node selector |
| apiGateway.podAnnotations | object | `{}` | Additional annotations to add to pods |
| apiGateway.podLabels | object | `{}` | Additional labels to add to pods |
| apiGateway.podSecurityContext | object | `{"fsGroup":65532}` | Pod security context |
| apiGateway.registry | string | `"istaridigital.jfrog.io/customer-docker"` | Registry URL for images. The combination of registry, image, and tag will be used to pull the image. |
| apiGateway.replicaCount | int | `2` | Replica count. Defaults to 2, unlike the other services: the API Gateway is the single entry point for all routed traffic, so one pod failing (or rolling during an upgrade) must not sever every routed service at once. Whenever the effective minimum replica count (this value, or `autoscaling.minReplicas` when autoscaling) is 2 or more, the chart also creates a PodDisruptionBudget keeping at least one API Gateway pod up through node drains; at 1 replica no budget is created, so drains are never blocked. |
| apiGateway.resources.limits.cpu | string | `nil` | CPU limit (e.g. `500m`). Consider leaving unset — a throttled API Gateway slows every routed request. |
| apiGateway.resources.limits.memory | string | `nil` | Memory limit (e.g. `256Mi`). |
| apiGateway.resources.requests.cpu | string | `nil` | CPU request (e.g. `100m`). Required for CPU-based autoscaling. |
| apiGateway.resources.requests.memory | string | `nil` | Memory request (e.g. `128Mi`). Required for memory-based autoscaling. |
| apiGateway.serviceAccountAnnotations | object | `{}` | Additional annotations to apply to the service account |
| apiGateway.serviceAnnotations | object | `{}` | Additional annotations to apply to the service, note the following annotations for duplicate keys. |
| apiGateway.serviceType | string | `"ClusterIP"` | Service Type. Available options are ClusterIP, NodePort, LoadBalancer, ExternalName. |
| apiGateway.tag | string | `"2.11.4"` | Image tag. The combination of registry, image, and tag will be used to pull the image. |
| apiGateway.tolerations | list | `[]` | Tolerations. Example:  ``` tolerations: - "effect": "NoSchedule"   "key": "istari.k8s.io/role"   "operator": "Equal"   "value": "main" ``` |
| apiGateway.topologySpreadConstraints | list/null | `null` (chart-managed soft spread across nodes) | Topology spread constraints for API Gateway pods. Leave unset (`null`) for the chart default: a soft preference (`whenUnsatisfiable: ScheduleAnyway`) to spread replicas across nodes, so one node failure doesn't take every API Gateway pod down while clusters with a single node still schedule normally. Set `[]` to disable, or supply your own list — entries must include their own `labelSelector`. |
| apiGateway.tracing.enabled | bool/null | `null` (automatic: follows `jaeger.enabled`) | Whether the API Gateway records a trace for each request it proxies. Leave unset (`null`) for automatic behavior: tracing turns on when `jaeger.enabled` is true and stays off otherwise — no other configuration is needed. Set `false` to keep tracing off even with Jaeger deployed, or `true` (advanced) to export traces to your own OpenTelemetry collector, which also requires providing `OTEL_EXPORTER_OTLP_ENDPOINT` — via `apiGateway.env`, a ConfigMap in `apiGateway.extraEnvConfigMaps`, or a Secret in `apiGateway.extraEnvSecrets`. If you rely on a ConfigMap or Secret, make sure it actually contains `OTEL_EXPORTER_OTLP_ENDPOINT`: the chart cannot inspect their contents, and without the variable traces are silently dropped. The chart exports over gRPC by default; a collector that only accepts OTLP/HTTP additionally needs `OTEL_EXPORTER_OTLP_PROTOCOL: http/protobuf` in `apiGateway.env`. |
| apiGateway.virtualService.annotations | object | `{}` | Annotations on the VirtualService. |
| apiGateway.virtualService.enabled | bool | `false` | Create an Istio VirtualService for this service. Requires Istio installed in the cluster with the `networking.istio.io/v1` CRD (Istio 1.22+). |
| apiGateway.virtualService.gateways | list | `[]` | `spec.gateways[]` — Gateway resources to attach to. Use `<namespace>/<gateway-name>` to reference a Gateway in another namespace. Leave empty for mesh-internal traffic only. |
| apiGateway.virtualService.hosts | list | `[]` | `spec.hosts[]` — DNS names this VirtualService matches (FQDNs, or short names for mesh-internal traffic). Empty by default so that enabling the VirtualService without supplying your real hostname(s) fails the render instead of shipping a route for a hostname that isn't yours. |
...
| docs.tag | string | `"6.14.3"` | Image tag. The combination of registry, image, and tag will be used to pull the image. |
...
| fileservice.migrations.env | list | `[]` | Extra environment variables for the migration workloads only (the `db-migrate` Job, or the `init-db` init container when `runAsJob` is `false`), rendered after the service-level `env` — on duplicate names, these win. |
...
| identity.agentRegistration.resources | object | `{}` | Resources for the registration Job containers. |
| identity.autoscaling.cpuUtilization | int | `80` | Average CPU utilization percentage. Set to `null` to disable. |
| identity.autoscaling.enabled | bool | `false` | Enable/Disable autoscaling |
...
| identity.ingress.className | string | `""` | `ingressClassName` on the Ingress. Leave empty to use the cluster's default IngressClass. |
| identity.ingress.enabled | bool | `false` | Create a Kubernetes Ingress for this service. The cluster must have an Ingress controller (nginx, ALB / EKS Auto Mode, GCE, Traefik, etc.) that watches the chosen IngressClass. |
| identity.ingress.hosts | list | `[{"host":"identity.istari.customer-domain.com","paths":[{"path":"/","pathType":"Prefix"}]}]` | One entry per `spec.rules[]`. `host` is optional — when omitted, the rule matches any host. |
| identity.ingress.labels | object | `{}` | Additional labels on the Ingress (in addition to the standard Identity Service labels). |
| identity.ingress.servicePort | int | `80` | Service port the Ingress targets. Defaults to 80. |
| identity.ingress.tls | list | `[]` | TLS configuration; passed through to `spec.tls[]` verbatim. |
| identity.migrations.autoCleanupSuccessfulJob | bool | `true` | Automatically clean up the successful migration hook `Job` by including **`hook-succeeded`** in `helm.sh/hook-delete-policy` (alongside `before-hook-creation`). When `true`, Helm may remove the Job after the migration succeeds (less clutter; logs are shorter-lived on the cluster). When `false`, only `before-hook-creation` is set, so the completed Job (and its Pods) remain until the next install or upgrade replaces the hook—useful for auditing or inspecting migration logs. |
| identity.migrations.backoffLimit | int | `0` | `spec.backoffLimit` for the migration `Job` (number of retries after a failed Pod). `0` means no retries. |
| identity.migrations.env | list | `[]` | Extra environment variables for the migration workloads only (the `db-migrate` Job, or the `init-db` init container when `runAsJob` is `false`), rendered after the service-level `env` — on duplicate names, these win. |
| identity.migrations.podAnnotations | object | `{}` | Annotations for the migration Job Pod template only (e.g. `sidecar.istio.io/inject: "false"` to disable Istio sidecar injection). |
| identity.migrations.podLabels | object | `{}` | Extra labels for the migration Job Pod template only (in addition to the standard Identity Service labels). |
...
| identity.virtualService.gateways | list | `[]` | `spec.gateways[]` — Gateway resources to attach to. Use `<namespace>/<gateway-name>` to reference a Gateway in another namespace. Leave empty for mesh-internal traffic only. |
| identity.virtualService.hosts | list | `["identity.example.com"]` | `spec.hosts[]` — DNS names this VirtualService matches (FQDNs, or short names for mesh-internal traffic). The default below is an EXAMPLE — you MUST replace it with your actual hostname(s) before enabling the VirtualService. Clearing this list while `enabled: true` will cause the chart to fail to render. |
| identity.virtualService.labels | object | `{}` | Additional labels on the VirtualService (in addition to the standard Identity Service labels). |
...
| jaeger.enabled | bool | `false` | Enable / Disable the Jaeger subchart. When `false`, the subchart is not rendered at all and no Jaeger-dependent `OTEL_*` defaults are injected into the OTEL-SDK services (fileservice, identity, mcp, secure-connection). The api-gateway proxy is the exception: with `apiGateway.tracing.enabled: true` it still injects its own `OTEL_SERVICE_NAME`, `OTEL_RESOURCE_ATTRIBUTES`, and `OTEL_EXPORTER_OTLP_PROTOCOL` (e.g. to point at an external collector; the OTLP endpoint is then yours to supply via `apiGateway.env` / a ConfigMap / a Secret). The downward-API building blocks — `POD_NAME`, `POD_NAMESPACE`, `POD_UID`, `CONTAINER_NAME` — are still injected into those services and the api-gateway proxy regardless, so their app code can rely on them. |
...
| jaeger.persistence.size | string | `"20Gi"` | Size of the Badger PVC (raise `jaeger.jaeger.resources` memory alongside it — see there). Retention is TIME-based only (`ttl.spans` in `jaeger.userconfig`, default 72h): Badger does NOT evict old data under disk pressure. A full volume stops trace collection and can wedge Badger's garbage collection (reclaiming space itself needs free disk), potentially crash-looping the Pod. Size with headroom — 20Gi over 72h ≈ 80 KB/s of stored spans, and expired data is reclaimed lazily — and alert on PVC usage (`kubelet_volume_stats_used_bytes`) around 80%. Increasing this value expands the volume in place when the StorageClass allows expansion; to reduce trace volume, lower `ttl.spans` or sample at the source. Trace history is disposable: deleting the PVC and restarting the Pod is a valid recovery. |
...
| secureConnection.migrations.env | list | `[]` | Extra environment variables for the migration workloads only (the `db-migrate` Job, or the `init-db` init container when `runAsJob` is `false`), rendered after the service-level `env` — on duplicate names, these win. |

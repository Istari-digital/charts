...
![Version: 5.6.2](https://img.shields.io/badge/Version-5.6.2-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 11.x.x](https://img.shields.io/badge/AppVersion-11.x.x-informational?style=flat-square)
...
| identity.agentRegistration.env | list | `[]` | Extra environment variables applied to BOTH containers of every agent Job (the `create-tenant` init container and the `register-agent` container), rendered after the service-level `env` — on duplicate names, these win. Use to add variables common to the Job, or to give the containers a correct OpenTelemetry identity that shadows the value inherited from `identity.env`. Note the single list is shared by both containers, so it cannot give each a distinct `k8s.container.name`. |
...
| identity.publicClientRegistration.env | list | `[]` | Extra environment variables for every `register-public-client` container, rendered after the service-level `env` — on duplicate names, these win. Use to add Job-specific variables, or to give the differently-named container a correct OpenTelemetry identity (`OTEL_RESOURCE_ATTRIBUTES` with `k8s.container.name=register-public-client`) that shadows the value inherited from `identity.env`. |
...
| identity.registryClientRegistration.env | list | `[]` | Extra environment variables for the `register-client` container, rendered after the service-level `env` — on duplicate names, these win. Use to add Job-specific variables, or to give the differently-named container a correct OpenTelemetry identity (`OTEL_RESOURCE_ATTRIBUTES` with `k8s.container.name=register-client`) that shadows the value inherited from `identity.env`. |

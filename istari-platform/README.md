...
| identityService.identityRouterAgentRegistration.enabled | bool | `false` | Whether to render the agent-registration Jobs. Off by default: enable only in environments where the target agents' public-key blobs have been provisioned into `identityService.secretName` (otherwise the hook Job fails the release with a missing-secret-key error). |

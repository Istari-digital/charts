...
| identityService.identityRouterAgentRegistration.agents | list | `[]` | Agents to provision, one Job each. Fields per entry: `name` (required, DNS-safe identifier used in the Job name); `tenantSlug` (required, identity-service tenant the agent belongs to); `keySecretKey` (required, the key within `identityService.secretName` holding the base64-encoded public-only agent credential blob); `tenantDisplayName` (optional human-readable tenant name); `providerName` + `providerTenantId` (optional, supplied together, map the tenant to an upstream IdP org — e.g. `zitadel` and the Zitadel org id — so the agent's token carries the resourceowner org for Zitadel-namespaced role lookups); `username` (optional upstream user id to bind the agent to a pre-existing IdP identity — required for agents that rely on an existing Zitadel project grant such as the secure-connection-service's `rss_service_user` role); `displayName` (optional agent name claim). |

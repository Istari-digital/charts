...
![Version: 3.25.0](https://img.shields.io/badge/Version-3.25.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 10.x.x](https://img.shields.io/badge/AppVersion-10.x.x-informational?style=flat-square)
...
>
> Setting `ISTARI_DIGITAL_API_URL` routes a client's registry traffic through `<base>/registry`. It does **not** turn on identity-service authentication. That is a separate, explicitly named toggle — `ISTARI_DIGITAL_IDENTITY_SERVICE_ENABLED`, off by default — and it requires the gateway base URL to also be set. Earlier releases inferred identity from the mere presence of the URL; that inference has been removed, so a client can route through the gateway while continuing to authenticate directly against Zitadel.
>

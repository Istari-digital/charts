...
| identity.bootstrap.registryFallback | bool | `false` | When `registryClientId` names no platform service, grant the registry's roles to every service labelled `registry`, or failing that to every confidential service with no component. Needs an identity-service image whose `grant-platform-role` has `-registry-fallback`. |
| identity.bootstrap.resources | object | `{}` | Resources for the bootstrap containers. |
| identity.bootstrap.tenants | object | (see fields below) | Map each upstream IdP organization to an identity-service tenant with `bootstrap-tenants`, in a pre-install and pre-upgrade hook. |
| identity.bootstrap.tenants.credentialEnv | string | `"ISTARI_DIGITAL_IDENTITY_SERVICE_ZITADEL_MANAGER_KEY"` | Env var holding the IdP credential. |
| identity.bootstrap.tenants.enabled | bool | `false` | Render the Job. Off by default. |
| identity.bootstrap.tenants.issuer | string | `""` | Issuer to enumerate. Empty uses `identity.oidc.issuer`. |
| identity.bootstrap.tenants.provider | string | `"zitadel"` | Upstream IdP provider to enumerate. Only `zitadel` is supported. |
| identity.bootstrap.tenants.slugPrefix | string | `"zitadel"` | Prefix for derived tenant slugs. |

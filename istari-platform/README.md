...
>
> When you set both `router.apiUrl` and `identity.clientIntegrationEnabled`, the chart emits the identity toggle to the registry service, the frontend, and MCP together. It never emits the toggle from `router.apiUrl` alone, because all three treat identity-enabled-without-a-gateway-URL as a fatal startup or config-load error.
>

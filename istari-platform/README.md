...
| identityService.databaseType | string | `""` | Backend database for identity-service: `postgres` or `dgraph`. When set, the chart injects `ISTARI_DIGITAL_IDENTITY_SERVICE_DATABASE_TYPE` into the identity env `ConfigMap`. Leave empty to omit it (the service defaults to `postgres`) or to supply it yourself via `env`/`secretName`; a raw `env`/secret entry overrides this. `helm` fails the render for any value other than `""`, `postgres`, or `dgraph`. |
| identityService.deploymentAnnotations | object | `{}` | Additional annotations to add to the deployment |
| identityService.dgraphMigrations.enabled | bool | `false` | Apply the Dgraph schema (`migrate-dgraph up`) as a Deployment `initContainer` before the main container starts. Enable for the Dgraph cutover (set `identityService.databaseType: dgraph`) so the Dgraph schema is present before the service reads/writes Dgraph. Requires `ISTARI_DIGITAL_IDENTITY_SERVICE_DGRAPH_URL` reachable from the env `ConfigMap`/`secretName` (set `identityService.dgraphUrl`, or supply it via `env`/`secretName`). The step is idempotent (the migration runner tracks applied migrations), so it is safe to leave enabled across deploys. This service does not migrate data from Postgres to Dgraph; only the schema is applied. |
| identityService.dgraphMigrations.resources | object | `migrations.resources` | Resources for the `init-dgraph` init container. Falls back to `migrations.resources` when unset, so the two migration containers stay in sync unless this is overridden. |
| identityService.dgraphUrl | string | `""` | Dgraph connection string for `dgraph` mode, e.g. `dgraph://<host>:9080`. When set, the chart injects `ISTARI_DIGITAL_IDENTITY_SERVICE_DGRAPH_URL` into the identity env `ConfigMap`, reaching both the main container and the `init-dgraph` init container. Leave empty to supply it via `env`/`secretName`. The `ConfigMap` is plaintext; route a credential-bearing URL through `secretName` instead. |

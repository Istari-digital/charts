...
| identityService.frontendClientRegistration | object | (see fields below) | Settings for the `register-client -public-client-id` one-shot hook Job (pre-install/pre-upgrade) that registers the frontend's public (browser, PKCE) OAuth client and its exact-match redirect allowlist in the identity-service ClientStore, so `/oauth2/authorize` can validate the presented `client_id` and `redirect_uri`. The Job is rendered only when BOTH `identityService.enabled` and `identityService.frontendClientRegistration.enabled` are `true`. It reads `ISTARI_DIGITAL_IDENTITY_SERVICE_FRONTEND_CLIENT_ID`, `ISTARI_DIGITAL_IDENTITY_SERVICE_FRONTEND_REDIRECT_URIS` (comma-separated), and `ISTARI_DIGITAL_IDENTITY_SERVICE_DATABASE_URL` from `identityService.secretName` (helm-stack terraform provisions the first two). No key material is involved — public clients have none. |
| identityService.frontendClientRegistration.autoCleanupSuccessfulJob | bool | `true` | Automatically clean up the successful registration hook `Job` by including **`hook-succeeded`** in `helm.sh/hook-delete-policy` (alongside `before-hook-creation`). |
| identityService.frontendClientRegistration.backoffLimit | int | `6` | `spec.backoffLimit` for the registration Job (number of retries after a failed Pod). |
| identityService.frontendClientRegistration.enabled | bool | `false` | Whether to render the registration Job. Off by default: only enable in environments where `identityService.secretName` carries `ISTARI_DIGITAL_IDENTITY_SERVICE_FRONTEND_CLIENT_ID` / `_FRONTEND_REDIRECT_URIS`; otherwise the hook Job fails the release with a missing-secret-key error. Also requires an identity-service image whose `register-client` supports `-public-client-id` (Istari-digital/identity-service#88), and `identityService.migrations.runAsJob=true` (so migrations run as a pre-upgrade hook before this one, guaranteeing the `client_type`/`redirect_uris` schema exists); rendering fails fast otherwise. |
| identityService.frontendClientRegistration.podAnnotations | object | `{}` | Annotations for the registration Job Pod template only (in addition to `sidecar.istio.io/inject: "false"`). |
| identityService.frontendClientRegistration.podLabels | object | `{}` | Extra labels for the registration Job Pod template only. |
| identityService.frontendClientRegistration.resources | object | `{}` | Resources for the registration Job container. |

...
| jaeger.persistence.enabled | bool | `true` | Create a PersistentVolumeClaim named `jaeger-badger` for Badger trace storage. The PVC is deleted on `helm uninstall` (trace history is disposable). Note: block-storage PVCs are zonal on AWS/Azure — the Jaeger Pod becomes pinned to the volume's availability zone (on Azure, a ZRS storage class avoids this). Disable only if you also switch `jaeger.userconfig` to in-memory storage and set `jaeger.jaeger.extraVolumes` / `jaeger.jaeger.extraVolumeMounts` to `[]` (see the `jaeger.userconfig` comment for the full recipe). |
| jaeger.persistence.size | string | `"10Gi"` | Size of the Badger PVC. Retention is TIME-based only (the 72h `ttl.spans` in `jaeger.userconfig`): Badger does NOT evict older data when space runs low. A full volume stops trace collection (span writes fail with ENOSPC; the UI keeps serving stored traces) and can wedge Badger's garbage collection — reclaiming space itself needs free disk — so the Pod may crash-loop until space is freed. Size for your span volume (10Gi over 72h ≈ 40 KB/s of stored span data) with headroom: TTL-expired data is reclaimed lazily, so on-disk usage sits above the live 72h of data. Alert on PVC usage (`kubelet_volume_stats_used_bytes` / `_capacity_bytes`) around 80%. Increasing this value expands the volume in place when the StorageClass has `allowVolumeExpansion: true` (shrinking is not supported); to reduce volume instead, lower `ttl.spans` or sample at the source via `fileservice.env` (`OTEL_TRACES_SAMPLER=parentbased_traceidratio` + `OTEL_TRACES_SAMPLER_ARG`). If the volume fills anyway, trace history is disposable: delete the `jaeger-badger` PVC and restart the Jaeger Pod for a fresh volume. |

...
| identity.publicClientRegistration.clients | list | `[]` | Public clients to register, one Job each. Fields per entry: `name` (required; used in the Job name and the `istari.digital/client` label, so it must be a valid Kubernetes label value — begin and end with a letter or digit, contain only letters, digits, hyphens, or underscores, at most 63 characters), `clientIdKey` (required; the secret key holding the client_id), `redirectUrisKey` (required; the key holding the comma-separated exact-match redirect allowlist), `secretName` (optional; the secret holding this client's `clientIdKey`/`redirectUrisKey` — defaults to `identity.secretName`; set to `zitadel-identity-service-env` when the istari-zitadel-configurator publishes them, DPLAT-653; `DATABASE_URL` always comes from `identity.secretName`). Example: `{name: frontend, clientIdKey: ISTARI_DIGITAL_IDENTITY_SERVICE_FRONTEND_CLIENT_ID, redirectUrisKey: ISTARI_DIGITAL_IDENTITY_SERVICE_FRONTEND_REDIRECT_URIS, secretName: zitadel-identity-service-env}`. |
| identity.publicClientRegistration.enabled | bool | `false` | Whether to render the registration Jobs. Off by default: only enable in environments where each listed client's `clientIdKey`/`redirectUrisKey` actually exist in the secret its entry reads (the entry's `secretName`, `identity.secretName` by default); otherwise the hook Jobs fail the release with a missing-secret-key error. Also requires an Identity Service image whose `register-client` supports `-public-client-id` (Identity Service 1.1.0 or later), and `identity.migrations.runAsJob=true` (so migrations run as a pre-install/pre-upgrade hook, weight 5, before these — guaranteeing the `client_type`/`redirect_uris` schema exists on fresh installs and upgrades alike); rendering fails fast otherwise. |
...
| identity.registryClientRegistration.clientSecretName | string | `""` | Secret holding `ISTARI_DIGITAL_IDENTITY_SERVICE_REGISTRY_CLIENT` (the public-only client blob). Empty means `identity.secretName`. Set to `zitadel-identity-service-env` when the istari-zitadel-configurator generates the credential (DPLAT-653). `DATABASE_URL` always comes from `identity.secretName`. |
| identity.registryClientRegistration.enabled | bool | `false` | Whether to render the registration Job. Off by default: only enable in environments where the registry-service's Identity Service integration is turned on (so the secret named by `clientSecretName` — `identity.secretName` by default — actually contains `ISTARI_DIGITAL_IDENTITY_SERVICE_REGISTRY_CLIENT`). Otherwise the hook Job fails the release with a missing-secret-key error. |
